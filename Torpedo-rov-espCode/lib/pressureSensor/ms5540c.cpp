...
float conv::mbarTommHg(float mbar) {
    return mbar * 750.06 / 100000;
}

float conv::mbarToAtm(float mbar) {
    return mbar / 1013.25;
}

float conv::mbarToPascal(float mbar) {
    return mbar * 100;
}
...
float ms5540c::getTemperature(SecondOrderCompensation secondOrder) const {
...
float ms5540c::getPressure(SecondOrderCompensation secondOrder) const {
