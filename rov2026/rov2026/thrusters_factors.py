...
class ThrustersFactors(Node):
    def __init__(self):
        ...
        self.current_level = 'LOW'               # default speed level
        ...
        self.get_logger().info("Thruster Factors Node Started")
        self.get_logger().info(f"Loaded factors: {json.dumps(self.factors)}")
    ...
    # In level_callback, add this to catch mismatches:
    def level_callback(self, msg):
        level = msg.data
        # self.get_logger().info(f"Received speed_level: '{level}'")
    ...
    def pwm_callback(self, msg):
        ...
        # 🔍 Debug: see which level is being used
        # self.get_logger().info(f"Applying factors for level: {level_str}")
